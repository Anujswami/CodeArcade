...
def run_quiz_module():
    ...
    def back_action():
        ...
        run_select_module()
        # Placeholder function for back action
    ...
    # Quiz questions and options (truncated for brevity)
    questions = [{
    ...
    # Timer variables
    timer = TIMER_DURATION * 1000  # Timer in milliseconds
    start_time = pygame.time.get_ticks()

    # Function to check the answer
    def check_answer(selected_option):
        nonlocal current_question_index, option_buttons, quiz_over, score, feedback, feedback_color, last_feedback_time, first_attempt, start_time
    ...
    while app_is_alive:
        ...
        timer_text = f"Time Left: {max(0, (TIMER_DURATION - elapsed_time // 1000))}"  # Display remaining time
        draw_text(timer_text, FONT_SIZE, (255, 0, 0), (width // 2, 50))  # Display timer
        ...
        if not quiz_over:
            if current_question_index < len(questions):
                ...
                for button in option_buttons:
                    button.draw()
                if elapsed_time >= timer:  # Check if time is up
                    feedback = "Time's up!"
                    feedback_color = (255, 0, 0)
                    start_time = pygame.time.get_ticks()  # Reset the timer
                    ...
                    if current_question_index < len(questions):
                        option_buttons = create_option_buttons(questions[current_question_index]["options"], height // 2 - 50)
                elif feedback and (current_time - last_feedback_time) // 1000 < feedback_display_time:
